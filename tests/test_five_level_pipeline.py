...
def test_complete_five_level_pipeline_flow():
    ...
    try:
        ...
        if donruss_set.get("set_page_url"):
            ...
            print(
                f"✅ Found Gary Carter card with {len(gary_carter_card['card_urls'])} URLs"
            )
        else:
        ...
        if gary_carter_card.get("card_urls"):
            ...
            for i, row_data in enumerate(sample_rows):
                print(
                    f"   - Grade {row_data.get('tag_grade', 'N/A')}: Cert {row_data.get('cert_number', 'N/A')}"
                )
        else:
...
def test_data_structure_consistency():
    ...
    try:
        result = extract_grade_rows_from_url(
            "https://my.taggrading.com/pop-report/Baseball/1989/Donruss/Gary Carter/53"
        )
...
def test_grade_row_field_extraction():
    ...
    try:
        ...
        print(
            f"      Grade: {first_row.get('tag_grade')} (should be grade like '8.5 NM MT+')"
        )
        print(
            f"      Cert: {first_row.get('cert_number')} (should be cert number like 'X3032464')"
        )
        print(
            f"      Report URL: {first_row.get('report_url')} (should be absolute URL)"
        )
        print(
            f"      Completed Raw: {first_row.get('completed_date_raw')} (should be date like '11-21-2023')"
        )
        print(
            f"      Completed ISO: {first_row.get('completed_date_iso')} (should be ISO date or None)"
        )
