...
def test_postgresql_features():
    ...
    try:
        with engine.connect() as conn:
            ...
            result = conn.execute(
                text("SELECT typname FROM pg_type WHERE typname = 'jsonb';")
            )
            ...
            result = conn.execute(
                text("SELECT amname FROM pg_am WHERE amname = 'gin';")
            )
            ...
            result = conn.execute(
                text("SELECT amname FROM pg_am WHERE amname = 'brin';")
            )
            ...
            result = conn.execute(
                text("SELECT EXISTS (SELECT 1 FROM pg_class WHERE relkind = 'p');")
            )
    ...
    try:
        ...
        for table_name in ["audit_logs", "categories", "years", "sets", "cards"]:
            if table_name in tables:
                ...
                logger.info(
                    f"Table {table_name} columns: {[col['name'] for col in columns]}"
                )
