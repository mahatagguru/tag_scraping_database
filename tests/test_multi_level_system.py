    ...
    try:
        ...
        if result["sets"]:
            ...
            for set_data in result["sets"]:
                ...
                if metrics:
                    ...
                    for key in metrics.keys():
                        metrics_stats["common_metric_keys"][key] = (
                            metrics_stats["common_metric_keys"].get(key, 0) + 1
                        )
            ...
            if metrics_stats["sets_with_metrics"] > 0:
                metrics_stats["average_metrics_per_set"] = (
                    total_metrics / metrics_stats["sets_with_metrics"]
                )

            print(
                f"✅ Sets with metrics: {metrics_stats['sets_with_metrics']}/{metrics_stats['total_sets']}"
            )
            print(
                f"✅ Average metrics per set: {metrics_stats['average_metrics_per_set']:.1f}"
            )

            print(f"\nMost common metric keys:")
            sorted_keys = sorted(
                metrics_stats["common_metric_keys"].items(),
                key=lambda x: x[1],
                reverse=True,
            )
