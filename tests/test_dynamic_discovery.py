...
def test_dynamic_category_discovery():
    ...
    try:
        ...
        assert (
            len(categories) > 0
        ), f"Expected to find categories, but found {len(categories)}"
        assert (
            len(categories) >= 32
        ), f"Expected at least 32 categories, but found {len(categories)}"
...
def test_database_storage():
    ...
    try:
        ...
        test_card = {
            ...
            "card_name": "TestCard",
            "card_urls": [
                "https://my.taggrading.com/pop-report/TestSport/2024/TestSet/TestCard"
            ],
...
def main():
    ...
    if passed == total:
        print(
            "🎉 ALL TESTS PASSED! The pipeline has full dynamic discovery capabilities!"
        )
        return 0
    else:
