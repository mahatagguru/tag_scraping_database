...
def test_complete_pipeline_flow():
    ...
    try:
        ...
        if donruss_set.get('set_page_url'):
            ...
            for card_data in sample_cards:
                print(f"   - {card_data['card_name']}: {len(card_data['card_urls'])} URLs")
        else:
            ...
            for field in required_fields:
                if field in total_sample:
                    if field in expected_nulls and total_sample[field] is None:
                        print(f"   ✅ {field}: {type(total_sample[field]).__name__} (expected None)")
                    elif field in expected_nulls and total_sample[field] is not None:
                        print(f"   ❌ {field}: should be None but is {total_sample[field]}")
                    else:
...
def main():
    ...
    if success_count == len(tests):
        print("🎉 ALL TESTS PASSED! The four-level scraping pipeline is working correctly.")
    else:
