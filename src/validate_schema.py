...
class SchemaValidator:
    ...
    def validate_table_structure(self) -> bool:
        ...
        if missing_tables:
            ...
            self._add_result(
                "info",
                "This is normal for a new project. Run 'python src/setup_database.py' to create the database schema.",
            )
        else:
    ...
    def validate_foreign_key_relationships(self) -> bool:
        ...
        for table_name, relationships in expected_relationships.items():
            ...
            for fk_column, value in relationships.items():
                ...
                fk_exists = any(
                    fk["constrained_columns"] == [fk_column]
                ...
                if not fk_exists:
                    self._add_result(
                        "errors",
                        f"Missing foreign key: {table_name}.{fk_column} -> {ref_table}.{ref_column}",
                    )
                    all_valid = False
                else:
                    self._add_result(
                        "passed",
                        f"Foreign key exists: {table_name}.{fk_column} -> {ref_table}.{ref_column}",
                    )
    ...
    def validate_unique_constraints(self) -> bool:
        ...
        for table_name, expected_columns in expected_uniques.items():
            ...
            for columns in expected_columns:
                ...
                unique_exists = any(
                    set(uq["column_names"]) == set(column_list) for uq in table_uniques
                )
                ...
                if not unique_exists:
                    self._add_result(
                        "warnings",
                        f"Missing unique constraint: {table_name}({', '.join(column_list)})",
                    )
                    all_valid = False
                else:
                    self._add_result(
                        "passed",
                        f"Unique constraint exists: {table_name}({', '.join(column_list)})",
                    )
    ...
    def validate_check_constraints(self) -> bool:
        ...
        for table_name, expected_checks_list in expected_checks.items():
            ...
            for check_condition in expected_checks_list:
                ...
                normalized_expected = (
                    check_condition.replace("'", "").replace('"', "").replace(" ", "")
                )
                ...
                if not check_exists:
                    self._add_result(
                        "warnings",
                        f"Missing check constraint: {table_name} - {check_condition}",
                    )
                    all_valid = False
                else:
                    self._add_result(
                        "passed",
                        f"Check constraint exists: {table_name} - {check_condition}",
                    )
    ...
    def validate_indexes(self) -> bool:
        ...
        expected_indexes = {
            ...
            "cards": [
                ...
                "ix_cards_active",
                "ix_cards_player",
                "ix_cards_cert_number",
            ],
            "grades": ["ix_grades_label", "ix_grades_value", "ix_grades_active"],
            "snapshots": [
                "ix_snapshots_captured_at",
                "ix_snapshots_source",
                "ix_snapshots_complete",
            ],
            "populations": [
                "ix_populations_carduid_snapshot",
                "ix_populations_grade",
                "ix_populations_count",
            ],
            "audit_logs": [
                "ix_audit_logs_created_at_desc",
                "ix_audit_logs_level",
                "ix_audit_logs_component",
                "ix_audit_logs_status",
                "ix_audit_logs_error_code",
        ...
        for table_name, expected_index_list in expected_indexes.items():
            ...
            for expected_index in expected_index_list:
                if expected_index not in existing_index_names:
                    self._add_result(
                        "warnings", f"Missing index: {table_name}.{expected_index}"
                    )
                    all_valid = False
                else:
                    self._add_result(
                        "passed", f"Index exists: {table_name}.{expected_index}"
                    )
    ...
    def validate_data_integrity(self) -> bool:
        ...
        integrity_checks = [
            ...
            (
                ...
                "cards",
                "card_uid",
                "Populations without valid card",
            ),
            (
                "populations",
                "grade_id",
                "grades",
                "id",
                "Populations without valid grade",
            ),
            (
                "populations",
                "snapshot_id",
                "snapshots",
                "id",
                "Populations without valid snapshot",
            ),
            (
                "category_totals",
                "category_id",
                "categories",
                "id",
                "Category totals without valid category",
            ),
        ...
        for (
            ...
            if (
                table_name not in self.inspector.get_table_names()
                or ref_table not in self.inspector.get_table_names()
            ):
                continue
            ...
            try:
                ...
                with get_session() as session:
                    ...
                    if orphaned_count > 0:
                        self._add_result(
                            "errors", f"{description}: {orphaned_count} records found"
                        )
                        all_valid = False
                    else:
                        self._add_result(
                            "passed", f"{description}: No orphaned records"
                        )
    ...
    def validate_logical_relationships(self) -> bool:
        ...
        logical_checks = [
            ...
            (
                ...
                "categories",
                "name",
                "Years index with invalid sport",
            ),
            ...
            (
                ...
                "years_index",
                ["sport", "year"],
                "Sets per year with invalid sport/year",
            ),
            ...
            (
                ...
                "sets_per_year",
                ["sport", "year", "set_title"],
                "Cards per set with invalid sport/year/set",
            ),
            ...
            (
                ...
                "cards_per_set",
                ["sport", "year", "set_title", "card_name"],
                "Card grade rows with invalid sport/year/set/card",
            ),
        ]
        ...
        for (
            ...
            try:
                ...
                ref_cols = (
                    [ref_columns] if isinstance(ref_columns, str) else ref_columns
                )

                join_conditions = " AND ".join(
                    [f"t1.{fk} = t2.{ref}" for fk, ref in zip(fk_cols, ref_cols)]
                )
                ...
                with get_session() as session:
                    ...
                    if orphaned_count > 0:
                        self._add_result(
                            "warnings", f"{description}: {orphaned_count} records found"
                        )
                        all_valid = False
                    else:
                        self._add_result(
    ...
    def validate_audit_logging(self) -> bool:
        ...
        if "audit_logs" not in self.inspector.get_table_names():
            ...
            self._add_result(
                "info",
                "Run 'python src/setup_database.py' to create the database schema",
            )
        ...
        table_columns = [
            col["name"] for col in self.inspector.get_columns("audit_logs")
        ]
        missing_columns = [col for col in required_columns if col not in table_columns]

        if missing_columns:
            self._add_result(
                "warnings", f"Missing audit log columns: {', '.join(missing_columns)}"
            )
            return False
        else:
    ...
    def run_validation(self) -> dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Validation complete. {passed_checks}/{total_checks} checks passed ({summary['success_rate']:.1f}%)"
            )

            if error_checks > 0:
                logger.error(
                    f"Found {error_checks} errors that need immediate attention"
                )

            if warning_checks > 0:
                logger.warning(
                    f"Found {warning_checks} warnings that should be reviewed"
                )
    ...
    def print_validation_report(self, summary: dict[str, Any]) -> None:
        ...
        if summary["errors"] == 0 and summary["warnings"] == 0:
            ...
            print(
                "💡 For new projects, missing tables are normal. "
