...
class Category(Base):
    ...
    # Relationships
    years = relationship(
        "Year", back_populates="category", cascade="all, delete-orphan"
    )
...
class Set(Base):
    ...
    year_id = Column(
        Integer, ForeignKey("years.id", ondelete="CASCADE"), nullable=False
    )
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "total_items IS NULL OR total_items >= 0", name="chk_sets_total_items"
        ),
...
class Card(Base):
    ...
    populations = relationship(
        "Population", back_populates="card", cascade="all, delete-orphan"
    )
...
class Population(Base):
    ...
    card_uid = Column(
        Text, ForeignKey("cards.card_uid", ondelete="CASCADE"), primary_key=True
    )
    grade_id = Column(
        Integer, ForeignKey("grades.id", ondelete="CASCADE"), primary_key=True
    )
...
class CategoryTotal(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "num_sets IS NULL OR num_sets >= 0", name="chk_category_totals_num_sets"
        ),
        CheckConstraint(
...
class YearTotal(Base):
    ...
    id = Column(Integer, primary_key=True)
    year_id = Column(
        Integer, ForeignKey("years.id", ondelete="CASCADE"), unique=True, nullable=False
    )
    num_sets = Column(Integer, nullable=True)
    total_items = Column(Integer, nullable=True)
    total_graded = Column(Integer, nullable=True)
    ...
    __table_args__ = (
        CheckConstraint(
            "num_sets IS NULL OR num_sets >= 0", name="chk_year_totals_num_sets"
        ),
        CheckConstraint(
            "total_items IS NULL OR total_items >= 0",
...
class SetTotal(Base):
    ...
    id = Column(Integer, primary_key=True)
    set_id = Column(
        Integer, ForeignKey("sets.id", ondelete="CASCADE"), unique=True, nullable=False
    )
    ...
    __table_args__ = (
        CheckConstraint(
            "num_cards IS NULL OR num_cards >= 0", name="chk_set_totals_num_cards"
        ),
        CheckConstraint(
            "total_items IS NULL OR total_items >= 0", name="chk_set_totals_total_items"
        ),
        CheckConstraint(
            "total_graded IS NULL OR total_graded >= 0",
...
class YearsIndex(Base):
    ...
    # Relationships
    sets = relationship(
        "SetsPerYear", back_populates="year_index", cascade="all, delete-orphan"
    )
...
class SetsPerYear(Base):
    ...
    year_index_id = Column(
        Integer, ForeignKey("years_index.id", ondelete="CASCADE"), nullable=True
    )
    sport = Column(Text, nullable=False)
    year = Column(Text, nullable=False)
    year_url = Column(Text, nullable=False)
    ...
    cards = relationship(
        "CardsPerSet", back_populates="set_per_year", cascade="all, delete-orphan"
    )
...
class CardsPerSet(Base):
    ...
    set_per_year_id = Column(
        Integer, ForeignKey("sets_per_year.id", ondelete="CASCADE"), nullable=True
    )
    sport = Column(Text, nullable=False)
    year = Column(Text, nullable=False)
    set_title = Column(Text, nullable=False)
    ...
    __table_args__ = (
        UniqueConstraint(
            "sport", "year", "set_title", "card_name", name="uq_sport_year_set_card"
        ),
    ...
    grade_rows = relationship(
        "CardGradeRows", back_populates="card_per_set", cascade="all, delete-orphan"
    )
...
class CardGradeRows(Base):
    ...
    card_per_set_id = Column(
        Integer, ForeignKey("cards_per_set.id", ondelete="CASCADE"), nullable=True
    )
    sport = Column(Text, nullable=False)
    year = Column(Text, nullable=False)
    set_title = Column(Text, nullable=False)
...
class TotalsRollups(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "scope IN ('sport', 'year', 'set', 'card')", name="chk_totals_rollups_scope"
        ),
