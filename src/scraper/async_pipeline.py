...
class AsyncScrapingPipeline:
    ...
    def __init__(self, config: AsyncPipelineConfig):
        ...
        self.bulk_ops: Optional[AsyncBulkOperations] = None
        self.scraper: Optional[AsyncWebScraper] = None
        self.cache: Optional[ScrapingCacheManager] = None
    ...
    async def scrape_categories(self, base_url: str = "https://my.taggrading.com/pop-report") -> List[Dict[str, Any]]:
    ...
    async def scrape_cards(self, sport: str, year: str, set_name: str) -> List[Dict[str, Any]]:
        ...
        if self.cache:
            ...
            if cached_cards:
                print(f"✅ Found {len(cached_cards)} cached cards for {sport} {year} {set_name}")
                return cached_cards
    ...
    async def process_cards_batch(self, cards: List[Dict[str, Any]], sport: str, year: str, set_name: str) -> None:
        ...
        async def fetch_card_detail(url: str) -> Tuple[str, Optional[Dict[str, Any]]]:
            async with semaphore:
                try:
                    ...
                    if self.monitoring:
                        self.monitoring.metrics.record_scraping_error(str(type(e).__name__), "card_details")
                    return url, None
                    
        # Process cards in batches
        tasks = [fetch_card_detail(url) for url in card_urls]
        ...
        print(f"✅ Successfully processed {successful_cards}/{len(card_urls)} card details")
        
    @profile_function("run_pipeline")
    async def run_pipeline(self, sports: Optional[List[str]] = None) -> Dict[str, Any]:
        """Run the complete scraping pipeline."""
        self.stats['start_time'] = datetime.datetime.now()
        print(f"🚀 Starting async scraping pipeline at {self.stats['start_time']}")
        print(f"Configuration: {self.config.max_concurrent_requests} concurrent requests, "
        ...
        try:
            ...
            self.stats['errors'] += 1
            if self.monitoring:
                self.monitoring.metrics.record_scraping_error(str(type(e).__name__), "pipeline")
            raise
...
async def main():
    ...
    parser.add_argument(
        "--sports",
        nargs="+",
        help="Specific sports to scrape (e.g., Baseball Hockey)"
    )
    parser.add_argument(
        "--year-filter",
        nargs="+",
        help="Filter to specific years"
    )
    parser.add_argument(
        "--set-filter",
        nargs="+",
        help="Filter to specific sets"
    )
    parser.add_argument(
        "--card-filter",
        nargs="+",
        help="Filter to specific cards"
    )
    ...
    async with AsyncScrapingPipeline(config) as pipeline:
        try:
            stats = await pipeline.run_pipeline(args.sports)
