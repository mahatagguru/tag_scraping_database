...
def map_columns_by_headers(header_row):
    ...
    for i, header in enumerate(headers):
        ...
        if 'rank' in header_text and 'grade' not in header_text and 'by' not in header_text:
            column_map['rank'] = i
...
def extract_grade_rows_from_card_page(html, sport, year, set_title, card_name, card_url):
    """
            ...
            if col_index < len(cells):
                ...
                if field == 'report_url':
                    ...
                    if field in ['chronology', 'chron_by_grade']:
                        ...
                        value_attr = cell.attributes.get('value', '')
                        if value_attr and value_attr != '0':  # Use value attribute if it's not the default '0'
...
def extract_grade_rows_from_url(card_url, set_title=None):
    ...
    if set_title is None:
        ...
        set_title = raw_set_title.split('?')[0] if '?' in raw_set_title else raw_set_title
    ...
    return extract_grade_rows_from_card_page(html, sport, year, set_title, card_name, card_url)
