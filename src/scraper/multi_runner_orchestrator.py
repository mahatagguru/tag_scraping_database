...
class MultiRunnerOrchestrator:
    ...
    def start_runner(self, config: RunnerConfig) -> RunnerStatus:
        ...
        runner_status = RunnerStatus(
            runner_id=config.runner_id, start_time=time.time(), status="starting"
        )
            ...
            with self.coordination_lock:
                if runner_id in self.runners:
                    ...
                    if exit_code == 0:
                        ...
                        logger.error(
                            f"❌ Runner {runner_id} failed with exit code {exit_code}"
                        )
                        if stderr:
    ...
    def monitor_runners(self, check_interval: float = 10.0):
        ...
        while not self.shutdown_event.is_set():
            try:
                ...
                completed_count = sum(
                    1 for r in self.runners.values() if r.status == "completed"
                )
                failed_count = sum(
                    1 for r in self.runners.values() if r.status == "failed"
                )
    ...
    def get_final_summary(self) -> Dict[str, Any]:
        ...
        for runner_id, status in self.runners.items():
            ...
            if status.status == "completed":
                ...
                if status.results:
                    summary["total_categories"] += status.results.get(
...
def main():
    ...
    parser.add_argument(
        "--concurrency", type=int, default=3, help="Concurrency per runner (default: 3)"
    )
    parser.add_argument(
        "--delay",
        type=float,
    ...
    parser.add_argument(
        "--max-retries", type=int, default=3, help="Max retries per runner (default: 3)"
    )
    parser.add_argument(
        "--retry-backoff",
        type=float,
    ...
    parser.add_argument(
        ...
        default="INFO",
        help="Log level per runner (default: INFO)",
    )
