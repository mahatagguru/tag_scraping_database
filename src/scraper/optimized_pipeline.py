...
def process_card_url(card_url, session, db_cat, db_year, db_set, snapshot_date):
    ...
    try:
        ...
        for row in card_details['table_rows']:
            ...
            population_count = int(completed.replace('-', '').replace('/', '').replace(' ', '')) if completed and completed.replace('-', '').replace('/', '').replace(' ', '').isdigit() else None
...
def process_set_parallel(session, db_cat, db_year, s, cat_name, year, snapshot_date):
    ...
    try:
        ...
        total_graded = int(s['grades'][-1]) if s['grades'] and s['grades'][-1].isdigit() else None
        ...
        if card_urls:
            ...
            with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
                ...
                for card_url in card_urls:
                    future = executor.submit(process_card_url, card_url, session, db_cat, db_year, db_set, snapshot_date)
    ...
    try:
        ...
        for cat in categories:
            if cat['name'].strip().upper() == 'TOTALS':
                print(f"   CATEGORY TOTALS: sets: {cat.get('num_sets', 'N/A')} | items: {cat.get('total_items', 'N/A')} | graded: {cat.get('total_graded', 'N/A')}")
                continue
                
            print(f"   Category: {cat['name']} | img: {cat.get('image_url', 'N/A')}")
            
            try:
                db_cat = upsert_category(session, name=cat['name'], img=cat.get('image_url'))
                ...
                for y in years:
                    if y['year'].strip().upper() == 'TOTALS':
                        print(f"        YEAR TOTALS: sets: {y['num_sets']} | items: {y['total_items']} | graded: {y['total_graded']}")
                        continue
                        
                    print(f"        Year: {y['year']} | sets: {y['num_sets']} | items: {y['total_items']} | graded: {y['total_graded']}")
                    
                    try:
                        db_year = upsert_year(session, category_id=db_cat.id, year=int(y['year']))
                        
                        # Year total
                        total_graded = int(y['total_graded']) if y['total_graded'] and y['total_graded'].isdigit() else None
                        if total_graded is not None:
                            upsert_population_report(
                                session,
                                card_uid='ALL',
                        ...
                        if y['num_sets'] or y['total_items'] or y['total_graded']:
                            upsert_year_total(
                                session,
                                year_id=db_year.id,
                                num_sets=int(y['num_sets']) if y['num_sets'].isdigit() else None,
                                total_items=int(y['total_items']) if y['total_items'].isdigit() else None,
                                total_graded=total_graded,
                            )
                        
                        # Get sets for this year
                        set_url = f"{BASE_URL}/pop-report/{cat['name'].title()}/{y['year']}"
                        print(f"          Fetching sets for {cat['name']} {y['year']}...")
                        ...
                        with ThreadPoolExecutor(max_workers=8) as set_executor:
                            for s in sets:
                                if s['set_name'].strip().upper() == 'TOTALS':
                                    print(f"            SET TOTALS: grades: {s['grades']}")
                                    continue
                                    
                                print(f"            Set: {s['set_name']} | grades: {s['grades']}")
                                future = set_executor.submit(process_set_parallel, session, db_cat, db_year, s, cat['name'], y['year'], snapshot_date)
                        ...
                        print(f"          ✓ Processed {total_cards} total cards for {cat['name']} {y['year']}")
