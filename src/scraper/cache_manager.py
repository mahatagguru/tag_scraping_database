...
class CacheManager:
    ...
    async def get_stats(self) -> Dict[str, Any]:
        ...
        memory_size = sum(len(str(data).encode()) for data, _ in self._memory_cache.values())
        
        file_entries = len([f for f in os.listdir(self.cache_dir) if f.endswith('.cache')])
...
class ScrapingCacheManager:
    ...
    async def get_categories(self, sport: str) -> Optional[List[Dict[str, Any]]]:
        """Get cached categories for sport."""
        return await self.cache.get(f"categories_{sport}", self.CATEGORIES)
        
    async def set_categories(self, sport: str, categories: List[Dict[str, Any]]) -> None:
    ...
    async def get_years(self, sport: str) -> Optional[List[Dict[str, Any]]]:
    ...
    async def set_years(self, sport: str, years: List[Dict[str, Any]]) -> None:
        ...
        await self.cache.set(
            ...
            years, 
            self.YEARS,
            self.ttl_settings[self.YEARS]
        )
        
    async def get_sets(self, sport: str, year: str) -> Optional[List[Dict[str, Any]]]:
    ...
    async def set_sets(self, sport: str, year: str, sets: List[Dict[str, Any]]) -> None:
        ...
        await self.cache.set(
            f"sets_{sport}_{year}", 
            sets, 
            self.SETS,
            self.ttl_settings[self.SETS]
        )
        
    async def get_cards(self, sport: str, year: str, set_name: str) -> Optional[List[Dict[str, Any]]]:
        """Get cached cards for sport/year/set."""
        return await self.cache.get(f"cards_{sport}_{year}_{set_name}", self.CARDS)
        
    async def set_cards(self, sport: str, year: str, set_name: str, cards: List[Dict[str, Any]]) -> None:
    ...
    async def get_card_details(self, card_url: str) -> Optional[Dict[str, Any]]:
        """Get cached card details."""
        return await self.cache.get(f"card_details_{hashlib.md5(card_url.encode()).hexdigest()}", self.CARD_DETAILS)
...
_scraping_cache: Optional[ScrapingCacheManager] = None
