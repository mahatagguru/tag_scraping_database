...
class BulkDatabaseOperations:
    ...
    async def _bulk_upsert_years_impl(
        ...
        try:
            ...
            # Get IDs separately
            id_query = "SELECT id FROM years WHERE category_id = :category_id AND year = :year"
            ids = []
            for year in years:
    ...
    async def _bulk_upsert_sets_impl(
        ...
        try:
            ...
            # Get IDs separately
            id_query = "SELECT id FROM sets WHERE year_id = :year_id AND set_name = :set_name"
            ids = []
            for set_item in sets:
                result = await session.execute(
                    text(id_query), 
    ...
    async def _bulk_upsert_cards_impl(
        ...
        try:
            ...
            ids = []
            for card in cards:
                result = await session.execute(text(id_query), {'card_uid': card['card_uid']})
                row = result.fetchone()
                if row:
                    ids.append(row[0])
            return ids
