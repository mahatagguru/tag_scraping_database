...
def extract_sets_from_year_page(
    html: str, sport: str, year: str, year_url: str
) -> Dict[str, Any]:
    """
    ...
    for url in test_urls:
        ...
        try:
            ...
            for i, set_data in enumerate(result["sets"][:5]):  # Show first 5
                ...
                print(
                    f"     Metrics: {dict(list(set_data['metrics'].items())[:3])}..."
            ...
            for total_data in result["totals"]:
                print(
                    f"  {total_data['scope']} scope for {total_data['sport']} {total_data['year']}: {total_data['metrics']}"
                )
