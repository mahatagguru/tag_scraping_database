...
class AsyncWebScraper:
    ...
    def _needs_javascript(self, html: str) -> bool:
        ...
        has_content = any(
            re.search(pattern, html, re.IGNORECASE | re.DOTALL)
        ...
        for element in category_elements:
            try:
                ...
                if len(cells) >= 3:
                    try:
                        num_sets = (
                            int(cells[1].text(strip=True))
                            if cells[1].text(strip=True).isdigit()
                            else None
                        )
                        total_items = (
                            int(cells[2].text(strip=True))
                            if cells[2].text(strip=True).isdigit()
                            else None
                        )
                        total_graded = (
                            int(cells[3].text(strip=True))
                            if len(cells) > 3 and cells[3].text(strip=True).isdigit()
                            else None
                        )
                ...
                if len(cells) >= 3:
                    try:
                        row_data = {
                            ...
                            "cert_number": cells[3].text(strip=True)
                            if len(cells) > 3
                            else None,
                        }
