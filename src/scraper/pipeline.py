...
class PipelineStats:
    ...
    def print_summary(self):
        ...
        for table_name, stats in self.table_stats.items():
            ...
            if total_ops > 0:
                logger.info(
                    f"  {table_name}: {stats['inserts']} inserts, {stats['updates']} updates"
                )
...
def retry_with_backoff(
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            logger.warning(
                f"Attempt {attempt + 1} failed: {e}. Retrying in {backoff:.1f}s..."
            )
            time.sleep(backoff)
...
def discover_categories() -> List[str]:
    ...
    try:
        ...
        for category in potential_categories:
            try:
                ...
                for link in year_links:
                    ...
                    if (
                        f"/pop-report/{category}/" in href
                        or f"/pop-report/{encoded_category}/" in href
                    ) and href.count("/") == 3:
                        year_count += 1
        ...
        if working_categories:
            logger.info(
                f"Discovered {len(working_categories)} working categories: {working_categories}"
            )
            return working_categories
        else:
            logger.warning(
                "No working categories discovered dynamically. Using known categories."
            )
...
class FiveTierPipeline:
    ...
    def __init__(
        self, concurrency: int = None, delay: float = 1.0, dry_run: bool = False
    ):
        """
        Initialize pipeline
        ...
        self.concurrency = concurrency or int(
            os.getenv("PIPELINE_MAX_CONCURRENCY", "3")
        )
        self.delay = delay
        ...
        logger.info(
            f"Pipeline initialized: concurrency={self.concurrency}, delay={delay}s, dry_run={dry_run}"
        )
    ...
    def log_upsert(
        self, table_name: str, unique_key: Dict[str, Any], operation: str = "upsert"
    ):
    ...
    def scrape_category_years(self, category: str) -> Dict[str, Any]:
        ...
        for year_data in result["years"]:
            self.log_upsert(
                "years_index", {"sport": year_data["sport"], "year": year_data["year"]}
            )
        ...
        for total_data in result["totals"]:
            self.log_upsert(
                "totals_rollups",
                {"scope": total_data["scope"], "sport": total_data["sport"]},
            )
        ...
        logger.info(
            f"Stored {len(result['years'])} years and {len(result['totals'])} totals for {category}"
        )
        return result
    ...
    def run_pipeline(
        ...
        categories: List[str],
        start_from: Literal[
            "category", "year", "set", "card", "graderows"
        ] = "category",
        ...
        logger.info(
            f"Filters: year={year_filter}, set={set_filter}, card={card_filter}"
        )
        ...
        logger.info(
            f"🚀 Starting dynamic discovery and scraping for {len(categories)} categories"
        )
        logger.info(f"   Categories: {categories}")
        ...
        try:
            summaries = scrape_multiple_sports(
                sport_urls, concurrency=self.concurrency, delay=self.delay
            )
...
def main() -> None:
    ...
    parser.add_argument(
        "--categories",
        nargs="+",
        help="Categories to scrape (default: auto-discover all)",
    )
    parser.add_argument(
        "--discover-categories",
        action="store_true",
        help="Explicitly discover categories (this is now the default behavior)",
    )
    parser.add_argument(
        "--start-from",
        choices=["category", "year", "set", "card", "graderows"],
        default="category",
    )
