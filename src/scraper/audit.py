...
try:
    ...
    print(
        "Warning: Database models not available, audit logging will be file-based only"
    )
...
class AuditLogger:
    ...
    def log_performance(self, operation: str, execution_time_ms: int, **kwargs):
        ...
        return self.log(
            "INFO",
            f"Performance: {operation} completed in {execution_time_ms}ms",
            operation=operation,
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                audit_logger.log_performance(
                    operation, execution_time_ms, status="SUCCESS"
                )
...
def audit_context(operation: str, component: str = None, **context):
    ...
    try:
        ...
        execution_time_ms = int((time.time() - start_time) * 1000)
        audit_logger.log_performance(
            operation, execution_time_ms, status="SUCCESS", **context
        )
        ...
        # Log error
        audit_logger.log_error_with_context(
            f"Operation {operation} failed", e, operation=operation, context=context
        )
        raise
...
def log_scraping_start(sport: str, year: str = None, set_name: str = None, **kwargs):
    ...
    return audit_logger.log_operation(
        "SCRAPING_START", status="STARTED", context=context, **kwargs
    )


def log_scraping_complete(
    sport: str, year: str = None, set_name: str = None, items_found: int = 0, **kwargs
):
    """Log the completion of a scraping operation."""
    audit_logger = get_audit_logger("scraper")
    context = {"sport": sport, "items_found": items_found}
    if year:
        context["year"] = year
    if set_name:
        context["set_name"] = set_name
    ...
    return audit_logger.log_operation(
        "SCRAPING_COMPLETE", status="SUCCESS", context=context, **kwargs
    )
...
def log_database_operation(operation: str, table: str, record_count: int = 0, **kwargs):
    ...
    context = {"table": table, "record_count": record_count}

    return audit_logger.log_operation(
        operation, status="SUCCESS", context=context, **kwargs
    )
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if exc_type is None:
            ...
            self.audit_logger.log_performance(
                self.operation, execution_time_ms, status="SUCCESS"
            )
        else:
            # Error occurred
            self.audit_logger.log_error_with_context(
                f"Operation {self.operation} failed", exc_val, operation=self.operation
            )
