...
class MultiLevelOrchestrator:
    ...
    def scrape_sport_years_index(self, sport_url):
        ...
        try:
            ...
            print(
                f"  Discovered {len(result['years'])} years: {[y['year'] for y in result['years']]}"
            )
            ...
            self.session.commit()
            print(
                f"Stored {len(result['years'])} years and {len(result['totals'])} totals for sport"
            )
    ...
    def scrape_year_sets(self, year_url):
        ...
        try:
            ...
            print(
                f"    Discovered {len(result['sets'])} sets: {[s['set_title'] for s in result['sets']]}"
            )
            ...
            print(
                f"    Stored {len(result['sets'])} sets and {len(result['totals'])} totals"
            )
    ...
    def scrape_set_cards(self, set_data):
        ...
        try:
            result = extract_cards_from_url(
                set_data["set_page_url"], set_data["set_title"]
            )

            # Validate discovered data
            if not result.get("cards"):
                print(
                    f"        Warning: No cards discovered for {set_data['set_title']}"
                )
                return {"cards": [], "totals": []}

            print(
                f"        Discovered {len(result['cards'])} cards: {[c['card_name'] for c in result['cards'][:5]]}{'...' if len(result['cards']) > 5 else ''}"
            )
            ...
            print(
                f"        Stored {len(result['cards'])} cards and {len(result['totals'])} totals"
            )
    ...
    def scrape_card_grade_rows(self, card_data):
        ...
        if not card_data.get("card_urls"):
            print(
                f"          No card URLs for {card_data['card_name']}, skipping grade rows"
            )
            return 0
        ...
        try:
            grade_rows = extract_grade_rows_from_url(
                primary_card_url, card_data["set_title"]
            )
            ...
            if report_urls_found > 0:
                print(
                    f"            Stored {len(grade_rows)} grade rows with {report_urls_found} report URLs"
                )
            else:
                print(
                    f"            Stored {len(grade_rows)} grade rows (no report URLs found)"
                )
    ...
    def scrape_sport_complete(self, sport_url):
            ...
            with ThreadPoolExecutor(max_workers=self.concurrency) as executor:
                ...
                for future in as_completed(future_to_set):
                    ...
                    try:
                        ...
                        print(
                            f"      Error processing set {set_data['set_title']}: {e}"
                        )
        else:
            print("Using sequential processing")
            for set_data in all_sets:
            ...
            with ThreadPoolExecutor(max_workers=self.concurrency) as executor:
                ...
                for future in as_completed(future_to_card):
                    ...
                    try:
                        ...
                        print(
                            f"        Error processing card {card_data['card_name']}: {e}"
                        )
        else:
            print("Using sequential processing")
            for card_data in all_cards:
        ...
        pages_scraped = (
            1
            + len(years_result["years"])  # Sport index page
            + len(all_sets)  # Year pages
            + len(all_cards)  # Set pages  # Card pages
        )
...
def scrape_sport(sport_url, concurrency=3, delay=1.0):
    ...
    with MultiLevelOrchestrator(
        concurrency=concurrency, delay_between_requests=delay
    ) as orchestrator:
...
def scrape_multiple_sports(sport_urls, concurrency=3, delay=1.0):
    ...
    for i, sport_url in enumerate(sport_urls):
        ...
        try:
            ...
            summaries.append({"sport_url": sport_url, "error": str(e)})
